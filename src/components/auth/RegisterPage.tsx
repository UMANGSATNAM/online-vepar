...
function getStrength(pwd: string) {
  ...
  if (s <= 4) return { score: 4, label: 'Strong', color: 'bg-blue-400' }
  return { score: 5, label: 'Very Strong', color: 'bg-blue-600' }
}
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      {/* Left Panel */}
      <div className="hidden lg:flex lg:w-[480px] xl:w-[560px] flex-col relative overflow-hidden bg-blue-700 dark:bg-blue-950">
        <div className="absolute inset-0 dot-pattern opacity-10" />
        <div className="absolute top-10 right-10 w-72 h-72 bg-teal-400/10 rounded-full blur-3xl animate-orb-2" />
        <div className="absolute bottom-10 left-10 w-56 h-56 bg-blue-300/10 rounded-full blur-3xl animate-orb-1" />
        <div className="relative z-10 flex flex-col h-full p-12">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <h1 className="text-4xl xl:text-5xl font-bold text-white leading-tight mb-4">
              Start your free<br /><span className="text-blue-200">14-day trial</span>
            </h1>
            <p className="text-blue-100/80 text-lg mb-10">No credit card required. Launch your store in minutes.</p>
            <div className="bg-white/10 rounded-2xl p-5 border border-white/10 mb-8">
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-4xl font-bold text-white">₹0</span>
                <span className="text-blue-200/70 text-sm">/month for 14 days</span>
              </div>
              <p className="text-blue-100/60 text-sm">Then from ₹999/mo. Cancel anytime.</p>
            </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10 overflow-y-auto">
        <motion.div className="w-full max-w-sm py-4" variants={cv} initial="hidden" animate="visible">
            ...
            <motion.div variants={iv} className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="name" placeholder="Your brand name" value={name} onChange={e => setName(e.target.value)} required
                  className="pl-9 h-11 rounded-xl border-border/60 focus:border-blue-500" />
              </div>
            </motion.div>
            ...
            <motion.div variants={iv} className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="phone" type="tel" placeholder="10-digit mobile number" value={phone} onChange={e => setPhone(e.target.value)} required
                  className="pl-10 h-11 rounded-xl border-border/60 focus:border-blue-500" />
              </div>
            </motion.div>
            ...
            <motion.div variants={iv} className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="password" type={show ? 'text' : 'password'} placeholder="Min. 12 characters" value={password}
                  ...
                  className="pl-9 pr-10 h-11 rounded-xl border-border/60 focus:border-blue-500" />
              ...
              {password && (
                <div className="space-y-1 pt-0.5">
                  ...
                  <p className={`text-[11px] font-medium ${strength.score <= 2 ? 'text-red-500' : strength.score <= 3 ? 'text-amber-500' : 'text-blue-600'}`}>
                    {strength.label}
                  </p>
                </div>
              )}
            ...
            <motion.div variants={iv} className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="confirm" type="password" placeholder="Repeat your password" value={confirm}
                  ...
                  className="pl-9 pr-10 h-11 rounded-xl border-border/60 focus:border-blue-500" />
                {confirm && password === confirm && <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-blue-500" />}
              </div>
            </motion.div>
          ...
          <motion.div variants={iv} className="mt-6 text-center text-sm text-muted-foreground">
            ...
            <button onClick={() => setView('login')} className="text-blue-600 font-semibold hover:text-blue-700 transition-colors">Sign in</button>
          </motion.div>
        </motion.div>
      </div>
    </div>
